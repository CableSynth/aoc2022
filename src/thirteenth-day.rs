...
fn main() {
    let mut input = INPUT.lines().filter(|l| !l.is_empty()).map(|l| serde_json::from_str::<Value>(l).unwrap()).collect_vec();
    
    let part_1 = input.iter().tuples().positions(|(a, b)| compare(a, b) != Ordering::Greater).map(|i| i + 1).sum::<usize>();
    let dividers = [
    ...
    let part_2 = input.iter().positions(|b| dividers.contains(b)).map(|i| i + 1).product::<usize>();
